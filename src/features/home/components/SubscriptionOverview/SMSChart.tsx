        ...
        {payload.map((item: any) => {
          return (
            <div key={`tooltip-item-$${item.dataKey}`} className="tooltip-item">
              {t("common:dashboard.sms-sent")} {item.value}
            </div>
          );
        })}
      </TooltipWrapper>
  ...
  return (
    <Wrapper>
      <Typography typographyType="label" as="span">
        {t("common:dashboard.sms-notifications")}
      </Typography>
      <StyledCard>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={adminDashboardData}>
            <XAxis dataKey="dateTime" height={20} />
            <YAxis dataKey="SMS_SENT" width={20} allowDecimals={false} />
            <Tooltip content={renderTooltip} />
            <Line type="monotone" dataKey="SMS_SENT" stroke="#3a80f8" strokeWidth={1} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </StyledCard>
    </Wrapper>
